...
export async function downloadBinary(version: string, options: DownloadOptions = {}): Promise<string> {
  ...
  try {
    ...
    if (!tarballResp.ok) {
      ...
      throw new Error(
        `donsetch does not ship a ${PLATFORM.key} binary for v${version} (release fetch returned HTTP ${tarballResp.status}). Supported platforms: linux-x64, linux-arm64, linux-musl, darwin-x64, darwin-arm64, win32-x64.`,
      )
    }
